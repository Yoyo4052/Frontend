...
const ResultsFODA = () => {
  ...
  const getResults = async () => {
    ...
    const response = await fetch(`http://127.0.0.1:8000/verResultadosFODA/${id}/`, requestOptions);
  ...
  const deleteResultFODA = async (resultadoFODA_id) => {
    ...
    const response = await fetch(`http://127.0.0.1:8000/EliminarResultadoFODA/${resultadoFODA_id}/`, requestOptions);
    const data = await response.json();
