...
const Predict = () => {
  ...
  const getQuestions = async () => {
    const response = await fetch(`https://back-rendimiento-estudiantes.onrender.com/verCuestionario/6`);
  ...
  const registerAnswers = async () => {
    ...
    for (let i = 0; i < questions.length; i++) {
      ...
      const response = await fetch(`https://back-rendimiento-estudiantes.onrender.com/registrarRespuestas/`, requestOptions);
      if (!response.ok) {
        console.error("Error:", await response.json());
      }
    }

    const resultResponse = await fetch(`https://back-rendimiento-estudiantes.onrender.com/gemini/${id}/`);
