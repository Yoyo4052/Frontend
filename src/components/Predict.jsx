...
const Predict = () => {
  ...
  const getQuestions = async () => {
    const response = await fetch(`http://127.0.0.1:8000/verCuestionario/6`);
  ...
  const registerAnswers = async () => {
    ...
    for (let i = 0; i < questions.length; i++) {
      ...
      const response = await fetch(`http://127.0.0.1:8000/registrarRespuestas/`, requestOptions);
      if (!response.ok) {
        console.error("Error:", await response.json());
      }
    }

    const resultResponse = await fetch(`http://127.0.0.1:8000/gemini/${id}/`);
